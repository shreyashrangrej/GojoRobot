...
from GojoRobot import pbot as LYCIA
...
@LYCIA.on_message(filters.command("Gojo"))
async def Lycia(_, message):
    if len(message.command) < 2:
        await message.reply_text("Lycia AI Voice Chatbot")
        return
    text = message.text.split(None, 1)[1]
    Gojo = text.replace(" ", "%20")
    m = await message.reply_text("Gojo Is Best...")
    try:
        L = await fetch(f"https://api.affiliateplus.xyz/api/chatbot?message={lycia}&botname=Gojo&ownername=Abhishek&user=1")
        chatbot = L["message"]
